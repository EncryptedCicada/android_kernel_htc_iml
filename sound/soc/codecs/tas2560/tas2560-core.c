...
#ifdef TAS2560_HAPTICS
	TAS2560_DR_BOOST_REG_2,	0x01,	0x04,
	TAS2560_BOOST_CTRL,		0x01,	0x03,
	TAS2560_BST_ALWAYSON_REG, 0x01, 0x04,
#else
		...
		if (nRegister == TAS2560_MSLEEP) {
			...
			if (nRegister != 0xFFFFFFFF) {
				...
				if (nSize > 1) {
					...
					nResult = pTAS2560->bulk_write(pTAS2560, chn, nRegister, Buf, nSize);
					if (nResult < 0)
						break;
				} else if (nSize == 1) {
					nResult = pTAS2560->write(pTAS2560, chn, nRegister, nData[0]);
					if (nResult < 0)
						break;
				} else {
					dev_err(pTAS2560->dev,
...
int tas2560_set_ASI_fmt(struct tas2560_priv *pTAS2560, unsigned int fmt)
{
	...
	pTAS2560->update_bits(pTAS2560, channel_both, TAS2560_ASI_CFG_1, TAS2560_DIRINV_MASK | 0x02,
			asi_cfg_1 | 0x02);
...
int tas2560_getLoad(struct tas2560_priv *pTAS2560, enum channel chn)
{
	int ret = -1;
	int value = -1;

	dev_dbg(pTAS2560->dev, "%s\n", __func__);
	pTAS2560->read(pTAS2560, chn, TAS2560_LOAD, &value);
...
int tas2560_parse_dt(struct device *dev, struct tas2560_priv *pTAS2560)
{
	...
	if (rc) {
		...
		dev_dbg(pTAS2560->dev, "ti,load=%d", pTAS2560->mnLeftLoad);
	}

	rc = of_property_read_u32(np, "ti,right-load", &pTAS2560->mnRightLoad);
	if (rc) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"ti,load", np->full_name, rc);
	} else {
		dev_dbg(pTAS2560->dev, "ti,load=%d", pTAS2560->mnRightLoad);
	}
	...
	pTAS2560->mnIRQGPIO1 = of_get_named_gpio(np, "ti,irq-gpio1", 0);
	if (!gpio_is_valid(pTAS2560->mnIRQGPIO1)) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"ti,irq-gpio1", np->full_name, pTAS2560->mnIRQGPIO1);
	} else {
		dev_dbg(pTAS2560->dev, "ti,irq-gpio1=%d", pTAS2560->mnIRQGPIO1);
	}

	pTAS2560->mnIRQGPIO2 = of_get_named_gpio(np, "ti,irq-gpio2", 0);
	if (!gpio_is_valid(pTAS2560->mnIRQGPIO2)) {
		dev_err(pTAS2560->dev, "Looking up %s property in node %s failed %d\n",
			"ti,irq-gpio2", np->full_name, pTAS2560->mnIRQGPIO2);
	} else {
		dev_dbg(pTAS2560->dev, "ti,irq-gpio2=%d", pTAS2560->mnIRQGPIO2);
	}
...
{
	return tas2560_i2c_load_data(pTAS2560, channel_both, p_tas2560_default_data);
}
...
int tas2560_load_platdata(struct tas2560_priv *pTAS2560)
{
	...
	if ((gpio_is_valid(pTAS2560->mnIRQGPIO1))&& (gpio_is_valid(pTAS2560->mnIRQGPIO2))) {
		nResult = tas2560_i2c_load_data(pTAS2560, channel_both, p_tas2560_irq_config);
		if (nResult < 0)
			goto end;
	}

	if (pTAS2560->mnPLL == PLL_BCLK_1P536_48KSR)
		nResult = tas2560_i2c_load_data(pTAS2560, channel_both, p_tas2560_48kSR_BCLK_1P536_data);
	else if (pTAS2560->mnPLL == PLL_BCLK_3P072_48KSR)
		nResult = tas2560_i2c_load_data(pTAS2560, channel_both, p_tas2560_48kSR_BCLK_3P072_data);
	if (nResult < 0)
		goto end;

	if ((pTAS2560->mnPLL == PLL_BCLK_1P536_48KSR)
		|| (pTAS2560->mnPLL == PLL_BCLK_3P072_48KSR))
		nResult = tas2560_i2c_load_data(pTAS2560, channel_both, p_tas2560_48khz_data);
	if (nResult < 0)
		goto end;

	nResult = tas2560_setLoad(pTAS2560, channel_left, pTAS2560->mnLeftLoad);
	if (nResult < 0)
		goto end;	
	nResult = tas2560_setLoad(pTAS2560, channel_right, pTAS2560->mnRightLoad);
	if (nResult < 0)
		goto end;
	...
	if (pTAS2560->mnASIFormat == TAS2560_DATAFORMAT_I2S) {
		...
		if (nResult < 0)
			goto end;
		nResult = pTAS2560->write(pTAS2560, channel_both, TAS2560_ASI_OFFSET_1, 0x00);
	} else if (pTAS2560->mnASIFormat == TAS2560_DATAFORMAT_DSP) {
		nResult = tas2560_set_ASI_fmt(pTAS2560,
			SND_SOC_DAIFMT_CBS_CFS|SND_SOC_DAIFMT_IB_IF|SND_SOC_DAIFMT_DSP_A);
		if (nResult < 0)
			goto end;
		nResult = pTAS2560->write(pTAS2560, channel_left, TAS2560_ASI_OFFSET_1, 0x01);
		if (nResult < 0)
			goto end;
		nResult = pTAS2560->write(pTAS2560, channel_right, TAS2560_ASI_OFFSET_1, 0x21);
	} else {
...
static int tas2560_load_postpwrup(struct tas2560_priv *pTAS2560,enum channel mchannel)
{
	int nResult = 0;

	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_idle_chnl_detect);
	if (nResult < 0)
		goto end;

#ifdef TAS2560_HAPTICS
	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_haptics_postpwrup);
	if (nResult < 0)
		goto end;

	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_disable_voltage_limiter);
	if (nResult < 0)
		goto end;
#else
	if (pTAS2560->mnPPG) {
		nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_PPG_data);
		if (nResult < 0)
			goto end;
	}
#endif

	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_HPF_data);
	if (nResult < 0)
		goto end;

#ifndef TAS2560_HAPTICS
	if (pTAS2560->mnLeftLoad == LOAD_8OHM) {
		nResult = tas2560_i2c_load_data(pTAS2560, channel_left, p_tas2560_8Ohm_data);
	} else if (pTAS2560->mnLeftLoad == LOAD_6OHM) {
		nResult = tas2560_i2c_load_data(pTAS2560, channel_left, p_tas2560_6Ohm_data);
	}
	if (nResult < 0)
		goto end;
	
	if (pTAS2560->mnRightLoad == LOAD_8OHM) {
		nResult = tas2560_i2c_load_data(pTAS2560, channel_right, p_tas2560_8Ohm_data);
	} else if (pTAS2560->mnRightLoad == LOAD_6OHM) {
		nResult = tas2560_i2c_load_data(pTAS2560, channel_right, p_tas2560_6Ohm_data);
	}
	if (nResult < 0)
		goto end;

	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_boost_headroom_data);
	if (nResult < 0)
		goto end;

	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_thermal_foldback);
	if (nResult < 0)
		goto end;

	nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_Vsense_biquad_data);
#endif
...
int tas2560_LoadConfig(struct tas2560_priv *pTAS2560, bool bPowerOn, enum channel echannel)
{
	int nResult = 0;
	...
	if (bPowerOn) {
		...
		nResult = tas2560_i2c_load_data(pTAS2560, echannel, p_tas2560_shutdown_data);
		if (nResult < 0)
			goto end;
	}
	...
	nResult = pTAS2560->write(pTAS2560, channel_both, TAS2560_SW_RESET_REG, 0x01);
	if (nResult < 0)
		goto end;
	msleep(1);
	...
	if (bPowerOn) {
		...
		nResult = tas2560_i2c_load_data(pTAS2560, echannel, p_tas2560_startup_data);
		if (nResult < 0)
			goto end;
		nResult = tas2560_load_postpwrup(pTAS2560,echannel);
		if (nResult < 0)
			goto end;
		nResult = tas2560_i2c_load_data(pTAS2560, echannel, p_tas2560_unmute_data);
		if (nResult < 0)
			goto end;
...
int tas2560_enable(struct tas2560_priv *pTAS2560, bool bEnable, enum channel mchannel)
{
	int nResult = 0;
	dev_dbg(pTAS2560->dev, "%s tas2560_enable\n", __func__);

	if (bEnable) {
		if (!pTAS2560->mbPowerUp[mchannel - 1]) {
			dev_dbg(pTAS2560->dev, "%s power up\n", __func__);
			pTAS2560->clearIRQ(pTAS2560);
			nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_startup_data);
			if (nResult < 0)
				goto end;
			nResult = tas2560_load_postpwrup(pTAS2560,mchannel);
			if (nResult < 0)
				goto end;
#ifndef TAS2560_HAPTICS
			nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_unmute_data);
#endif
			if (nResult < 0)
				goto end;
		...
		if (pTAS2560->mbPowerUp[mchannel-1]) {
			...
			if (hrtimer_active(&pTAS2560->mtimer))
				hrtimer_cancel(&pTAS2560->mtimer);
			pTAS2560->enableIRQ(pTAS2560, false);
			nResult = tas2560_i2c_load_data(pTAS2560, mchannel, p_tas2560_shutdown_data);
